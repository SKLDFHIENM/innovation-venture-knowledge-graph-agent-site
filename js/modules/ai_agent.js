...
📚 <strong>创新创业大赛 5 大指标深度拆解：</strong>
...
你是《大学生创新创业基础》国家级精品课程首席专家兼大奖赛总评委导师。
...
// ─── EventBus 多 Agent 协同分发器 (AgentDispatcher) ───
(function() {
    window.IEKG = window.IEKG || {};
    window.IEKG.modules = window.IEKG.modules || {};
    ...
    class AgentDispatcher {
        constructor() {
            this.bus = window.IEKG.eventBus;
            this.setupListeners();
        }

        setupListeners() {
            if (!this.bus) return;
            this.bus.on('agent:query', async (data) => {
                const { query, role, callback } = data;
                try {
                    this.bus.emit('agent:thinking', { query, role });
                    const response = await window.askDeepSeek(query);
                    this.bus.emit('agent:response', { query, role, response });
                    if (typeof callback === 'function') callback(null, response);
                } catch (err) {
                    this.bus.emit('agent:error', { query, role, error: err.message });
                    if (typeof callback === 'function') callback(err);
                }
            });
        }

        dispatch(role, query, callback) {
            if (this.bus) {
                this.bus.emit('agent:query', { role, query, callback });
            } else if (typeof window.askDeepSeek === 'function') {
                window.askDeepSeek(query).then(res => callback(null, res)).catch(callback);
            }
        }
    }

    window.IEKG.modules.agent = window.IEKG.modules.agent || {};
    window.IEKG.modules.agent.dispatcher = new AgentDispatcher();
})();
