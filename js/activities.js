    ...
    window.ACTIVITIES_DATA = [
        ...
        },
        {
            ...
            workflows: {
                ...
                student: '在风暴板上快速贴出脑洞 ➔ 尝试突破常规组合 ➔ 投票选出前 3 创想',
        ...
        },
        {
            ...
            category: 'feasibility',
            categoryName: '市场可行',
            title: '盈亏平衡点 (BEP) 临界销量快速测算',
            time: '10分钟',
            pblNode: '#42 财务保本测算',
            badge: '📈 BEP 临界测算',
            desc: '输入固定成本、变动成本与单价，快速计算项目保本销售量与月保本额。',
        ...
        },
        {
            ...
            badge: '🎬 10页路演 Deck',
            desc: '拆解优秀奖项目标准 10 页 Pitch Deck 逻辑链（痛点-方案-市场-产品-模式-壁垒-团队-财务）。',
            rules: '1. 匹配 10 页 Deck 逻辑骨架；2. 检查单页字数与视觉重点；3. 生成标准 Pitch 纲要。',
            workflows: {
                teacher: '展示国赛优秀奖项目 10 页 Deck 原件 ➔ 讲解一页一主旨原则 ➔ 指导视觉美化',
...
// 课堂自由定义小组数量与角色分工
window.applyCustomGroupCount = function() {
    const input = document.getElementById('custom-group-count');
    const count = parseInt(input ? input.value : 6, 10) || 6;

    let groupsData = JSON.parse(localStorage.getItem('iekg_classroom_groups') || '[]');
    if (groupsData.length < count) {
        for (let i = groupsData.length + 1; i <= count; i++) {
            groupsData.push({
                id: i,
                name: '第' + i + '小组',
                project: '未命名PBL项目' + i,
    ...
    } else if (groupsData.length > count) {
        groupsData = groupsData.slice(0, count);
    }

    localStorage.setItem('iekg_classroom_groups', JSON.stringify(groupsData));
    renderClassroomGroupsGrid();
};

window.renderClassroomGroupsGrid = function() {
    const grid = document.getElementById('classroom-groups-grid');
    if (!grid) return;

    let groupsData = JSON.parse(localStorage.getItem('iekg_classroom_groups') || '[]');
    if (!groupsData || groupsData.length === 0) {
        window.applyCustomGroupCount();
        return;
    }

    grid.innerHTML = groupsData.map((g, idx) => `
        <div style="background:var(--bg-subtle); border-radius:var(--radius-md); padding:14px; border:1px solid var(--border-color);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <input type="text" value="${g.name}" onchange="updateGroupField(${idx}, 'name', this.value)" style="font-weight:bold; font-size:14px; color:var(--primary-blue); border:none; background:transparent; width:60%;">
                <span style="font-size:11px; padding:2px 8px; border-radius:10px; background:var(--primary-light); color:var(--primary-blue); font-weight:bold;">组 ${g.id}</span>
            </div>
            <div style="margin-bottom:8px;">
                <input type="text" value="${g.project}" placeholder="项目名称" onchange="updateGroupField(${idx}, 'project', this.value)" style="font-size:12px; color:var(--text-muted); width:100%; border:1px dashed var(--border-color); padding:4px 6px; border-radius:4px;">
            </div>
            <div style="font-size:11px; display:grid; grid-template-columns: 1fr 1fr; gap:6px;">
                <div>👑 <strong>CEO:</strong> <input type="text" value="${g.ceo}" onchange="updateGroupField(${idx}, 'ceo', this.value)" style="width:100%; font-size:11px;"></div>
                <div>💻 <strong>CTO:</strong> <input type="text" value="${g.cto}" onchange="updateGroupField(${idx}, 'cto', this.value)" style="width:100%; font-size:11px;"></div>
                <div>📈 <strong>CMO:</strong> <input type="text" value="${g.cmo}" onchange="updateGroupField(${idx}, 'cmo', this.value)" style="width:100%; font-size:11px;"></div>
                <div>💰 <strong>CFO:</strong> <input type="text" value="${g.cfo}" onchange="updateGroupField(${idx}, 'cfo', this.value)" style="width:100%; font-size:11px;"></div>
            </div>
        </div>
    `).join('');
};

window.updateGroupField = function(idx, field, val) {
    let groupsData = JSON.parse(localStorage.getItem('iekg_classroom_groups') || '[]');
    if (groupsData[idx]) {
        groupsData[idx][field] = val;
        localStorage.setItem('iekg_classroom_groups', JSON.stringify(groupsData));
    }
};

document.addEventListener('DOMContentLoaded', () => {
    setTimeout(window.renderClassroomGroupsGrid, 300);
});
